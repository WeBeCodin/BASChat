...
export async function categorizeTransactions(
  ...
  try {
    ...
    if (transactions.length > BATCH_SIZE) {
      console.log(`Processing ${transactions.length} transactions in batches of ${BATCH_SIZE}`);
      ...
      for (let i = 0; i < transactions.length; i += BATCH_SIZE) {
        ...
        const batchResult = await processBatch({ ...input, rawTransactions: batch }, apiKey);
      ...
      return {
        ...
        maybeTransactions: allMaybeTransactions.length > 0 ? allMaybeTransactions : undefined
      };
    } else {
...
Return a JSON object with this exact structure:
{
  "transactions": [
    ...
    try {
      ...
      console.log("Raw categorized transactions from AI:", categorizedTransactions.length);
      ...
      console.log("processBatch returning:", JSON.stringify(finalResult, null, 2));
      console.log(`Batch processed: ${finalTransactions.length} certain, ${maybeTransactions.length} uncertain transactions`);
      return finalResult;
