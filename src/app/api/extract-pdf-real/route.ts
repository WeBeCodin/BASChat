...
export async function POST(request: NextRequest) {
  try {
    ...
    const pythonServiceUrl = process.env.PYTHON_EXTRACTOR_URL || 'http://localhost:8080';
    
    console.log('Calling Python service at:', pythonServiceUrl);
    
    const extractionResponse = await fetch(`${pythonServiceUrl}/extract-base64`, {
      method: 'POST',
      headers: {
    ...
    if (!extractionResponse.ok) {
      ...
      console.error('Python service error:', extractionResponse.status, errorText);
    ...
    const normalizedTransactions = transactions.map((tx: any) => ({
      ...
      description: tx.description || tx.merchant || tx.details || 'Unknown Transaction',
      amount: parseFloat(tx.amount || tx.value || 0)
    }));

    console.log('Returning normalized transactions:', normalizedTransactions.length);
    ...
    return NextResponse.json({
      ...
      transactions: [
        {
          date: "2025-07-15",
          description: "FALLBACK: Real PDF extraction failed - using sample data",
          amount: 0.00
        }
      ]
    });
