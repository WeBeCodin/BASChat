...
async function extractTransactionsFromPDF(buffer: Buffer) {
  try {
    ...
    console.log(
      `PDF extracted: ${data.numpages} pages, ${text.length} characters`
    );
      ...
      if (match) {
        try {
          ...
          let amount = parseFloat(
            amountStr.replace(/[$,\s]/g, "").replace(/[()]/g, "-")
          );
          ...
          transactions.push({
            ...
            balance: balanceStr
              ? parseFloat(balanceStr.replace(/[$,\s]/g, ""))
              : null,
          });
...
function categorizeTransaction(description: string, amount: number): string {
  ...
  if (amount > 0) {
    ...
    if (desc.includes("payment") || desc.includes("purchase"))
      return "Purchase";
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `Real PDF extraction completed: ${result.transaction_count} transactions from ${result.page_count} pages`
    );
