...
export async function POST(request: NextRequest) {
  try {
    ...
    // Try real PDF extraction first, fallback to mock if service unavailable
    ...
    let response;
    try {
      response = await fetch(`${PYTHON_SERVICE_URL}/extract-upload`, {
        method: "POST",
        body: pythonFormData,
      ...
      console.log("Falling back to mock data due to service unavailability");
      useMockData = true;
    }
    ...
    if (!useMockData && response && !response.ok) {
      ...
      console.log("Python service returned error, falling back to mock data");
      useMockData = true;
    }
